...
def after_request(response):
    ...
    response.headers["Content-Security-Policy"] = (
        f"default-src 'self'; base-uri 'none'; object-src 'none'; style-src 'self' 'unsafe-inline' fonts.googleapis.com; font-src 'self' fonts.gstatic.com; script-src 'self' 'nonce-{NONCE}' 'strict-dynamic'"
    )
